...
public class CategoryService {
    ...
    private boolean isCategoryExistByName(String categoryName){
        ...
        if (existCategory) {
            throw new ConflictException(String.format(ErrorMessages.AUTHOR_ALREADY_EXIST, categoryName));
        }else{
            return false;
        }
    }
    ...
    public ResponseMessage<CategoryResponse> updateCategoryById(Long id, CategoryRequest categoryRequest) {
        Category category = categoryHelper.isAuthorExistById(id);
            if (
                ...
                throw new ConflictException(String.format(ErrorMessages.AUTHOR_ALREADY_EXIST,categoryRequest.getCategoryName()));
            }
